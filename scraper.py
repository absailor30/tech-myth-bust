"""
Tech Myth-Bust daily scraper - FREE STACK (Groq + DuckDuckGo + GitHub CSV).

Pulls high-reach tech items from HN, Reddit, RSS feeds. Uses Groq
(llama-3.3-70b-versatile, free tier) for the flag pass and verify pass.
Verify pass uses DuckDuckGo as grounding. Writes results to a daily CSV
in the data/ folder, which the workflow commits back to the repo.

Cost: $0. No Google Sheets, no Apps Script, no webhooks.
...
CSV_FIELDS = [
    "date", "flagged", "title", "source", "url", "summary",
    "claim", "reach_signal", "myth_angle", "verify_verdict", "verify_reason",
]
...
def write_csv(today: str, rows: list[dict[str, Any]]) -> str:
    ...
    # Also update a stable "latest.csv" for easy phone access.
    ...
    with open(latest, "w", encoding="utf-8", newline="") as f:
        ...
        for r in rows:
            writer.writerow({k: r.get(k, "") for k in CSV_FIELDS})
    print(f"[csv] wrote {len(rows)} rows to {path} and {latest}")
    return path
